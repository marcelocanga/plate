...
//@      buf[index++] = val;
//@    else
//@      diag_mesg(diag.error,"Array<T,dim> , index out of bound:"<<index<<", Dim:"<<dim1<<std::endl);
//@
//@    return *this;
//@  }
//
...
//
  ...
  T& operator()(int i){
    if(i>=0 && i<dim1) return buf[i];
      ...
      diag_mesg(diag.error,"Array:() index out of bounds:"<<i<<":dim1:"<<dim1<<std::endl);
      return buf[0];
    }
  }
...
//
  ...
  T& operator()(int i, int j){
    if(dim2*i+j < dim1*dim2)
      ...
      diag_mesg(diag.error,"Array<T,2>:(i,j) Out of bounds:"<<i<<":"<<j<<std::endl);
      return buf[0];
    }
  }
...
//
  ...
  if(xvec.size() == 3 && yvec.size() == 3 && zvec.size() == 3){
    ...
    diag_mesg(diag.error,"Cross product of vectors with dim neq 3:"
...
//
  if(xvec.dim1 == yvec.dim1){
    ...
    diag_mesg(diag.error,"Dot product of vectors with different size\n");
//
//************************************************      done
//
  return dot;
...
//
  if(xmat.dim1 == ymat.dim1 && xmat.dim2 == ymat.dim2 ){
    ...
    diag_mesg(diag.error,"Dot product of matrices with different size\n");
//
//************************************************      done
//
  return dot;
...
//
  if(xvec.dim1 == yvec.dim1){
    ...
    diag_mesg(diag.error,"Distance of vectors with different size\n");
//
//************************************************      done
//
  return sqrt(dot);
...
void identity(Array<V,2>& matrix)
{
  if(matrix.size1() != matrix.size2()){
    diag_mesg(diag.error,"Identity: matrix is not square:"<<matrix.size1()<<" : "<<matrix.size2()<<std::endl);
    return;
  }
  matrix = d_zero;
...
int scale(V alpha, Array<V,1>& xvec, V beta,
  ...
  if( xvec.dim1 == yvec.dim1){
    ...
    diag_mesg(diag.error,"Array: Trying to scale two vectors with different size\n");
    return 1;
  }
}
...
int scale(V alpha, Array<V,2>& xmat, V beta,
  ...
  if( dim1 == ymat.dim1 &&  dim2 == ymat.dim2 ){
    ...
    diag_mesg(diag.error,"Array: Trying to scale two matrices with different size\n");
    return 1;
  }
}
	 ...
	 Array<T,1>& cvec, int cib)
{
  ...
  if(blast > bvec.size() || clast > cvec.size()){
    diag_mesg(diag.error,"Array::copy: Index out of bounds: bvec:"<<bvec.size()<<"    "<<cvec.size()<<std::endl);
    return 0;
  }
...
int copy( Array<V,2>& xmat, int ixbeg, int ixend,
  ...
  if(std::min(ixbeg,ixend)<0 || std::max(ixbeg,ixend) >= xmat.size1() ||
    ...
    diag_mesg(diag.error,"Array:copy:Index out of bounds: xmat:size1= "<<xmat.size1()<<" :"<<ixbeg<<","<<ixend<<std::endl<<
    ...
    diag_mesg(diag.error,"Array:copy:Number of objects differ:"<<
...
int copy( Array<V,1>& yvec, int iybeg, int iyend,
  ...
  if(std::min(ixbeg,ixend)<0 || std::max(ixbeg,ixend) >= xmat.size1() ||
     std::min(jxbeg,jxend)<0 || std::max(jxbeg,jxend) >= xmat.size2() ||
     std::min(iybeg,iyend)<0 || std::max(iybeg,iyend) >= yvec.size() ){
    diag_mesg(diag.error,"Array:copy:Index out of bounds:yvec.size= "<<yvec.size()<<" :"<<iybeg<<","<<iyend<<std::endl<<
...
int copy( Array<V,2>& xmat, int ixbeg, int ixend, int jxbeg, int jxend,
  ...
  if(std::min(ixbeg,ixend)<0 || std::max(ixbeg,ixend) >= xmat.size1() ||
    ...
    diag_mesg(diag.error,"Array:copy:Number of objects differ:"<<((ixend-ixbeg+1)*(jxend-jxbeg+1))
